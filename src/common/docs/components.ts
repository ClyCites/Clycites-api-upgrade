...
export const schemas: Record<string, OpenAPIV3_1.SchemaObject> = {
  ...
  ApiToken: {
    ...
    properties: {
      ...
      uiStatus: { type: 'string', enum: ['active', 'revoked', 'expired'] },
      expiresAt: isoDate,
  ...
  Organization: {
    ...
    properties: {
      ...
      slug: { type: 'string', example: 'buganda-farmers-cooperative' },
      type: { type: 'string', enum: ['enterprise', 'cooperative', 'government', 'individual'] },
      industry: { type: 'string', example: 'agriculture' },
      status: { type: 'string', enum: ['active', 'pending', 'suspended', 'archived'] },
      uiStatus: { type: 'string', enum: ['active', 'disabled'] },
      owner: mongoId,
  ...
  AdminRole: {
    ...
    properties: {
      ...
      organization: mongoId,
      permissions: { type: 'array', items: mongoId },
      level: { type: 'integer' },
  ...
  AdminModuleToggle: {
    type: 'object',
    properties: {
      workspaceId: { type: 'string', example: 'analytics' },
      enabled: { type: 'boolean' },
      status: { type: 'string', enum: ['enabled', 'disabled'] },
      uiStatus: { type: 'string', enum: ['enabled', 'disabled'] },
    },
  },
  ...
  ChartDefinition: {
    ...
    properties: {
      datasetId: {
        ...
        enum: [
          'market_sales_daily',
          'market_listings_activity',
          'market_price_trends',
          'market_demand_supply',
          'farmer_adoption',
          'farmer_profiles',
          'farm_production',
          'disease_cases_weekly',
          'disease_ai_performance',
          'disease_outbreaks',
          'weather_alerts_daily',
          'weather_risk_trends',
          'orders_performance',
          'orders_disputes',
          'expert_activity',
      ...
      metrics: {
        ...
        items: {
          ...
          required: ['type'],
          properties: {
            type: { type: 'string', enum: ['count', 'sum', 'avg', 'min', 'max', 'percent_change', 'rolling_avg_7d', 'rolling_avg_30d', 'distinct_count', 'rate'] },
      ...
      dimensions: {
        ...
        items: {
          type: 'object',
          properties: {
            type: { type: 'string', enum: ['time_hour', 'time_day', 'time_week', 'time_month', 'time_quarter', 'time_year', 'region', 'district', 'country', 'cropType', 'product', 'category', 'market', 'farmerId', 'organizationId', 'status', 'severity', 'alertType', 'expertId'] },
            field: { type: 'string' },
            alias: { type: 'string' },
          },
        },
      ...
      chartType: {
        type: 'string',
        enum: ['line', 'bar', 'stacked_bar', 'area', 'pie', 'donut', 'scatter', 'map', 'table', 'metric_card', 'heatmap', 'funnel'],
        example: 'line',
      },
      filters: {
        type: 'array',
        items: {
          ...
          properties: {
            field: { type: 'string' },
            operator: { type: 'string', enum: ['eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'in', 'not_in', 'contains', 'between'] },
            value: {},
          },
        },
      },
  ...
  Chart: {
    ...
    properties: {
      ...
      uiStatus: { type: 'string', enum: ['draft', 'published', 'archived'], example: 'draft' },
  ...
  Dashboard: {
    ...
    properties: {
      ...
      uiStatus: { type: 'string', enum: ['draft', 'published', 'archived'], example: 'draft' },
      tags: { type: 'array', items: { type: 'string' } },
      templateCategory: { type: 'string', enum: ['farmer', 'organization', 'expert', 'admin', 'outbreak', 'market'] },
      isTemplate: { type: 'boolean' },
      createdAt: isoDate,
    },
  },
  ...
  AnalyticsDataset: {
    type: 'object',
    properties: {
      id: { oneOf: [mongoId, { type: 'string', example: 'market_sales_daily' }] },
      datasetId: { type: 'string', example: 'market_sales_daily' },
      name: { type: 'string', example: 'Market Sales (Daily)' },
      label: { type: 'string', example: 'Market Sales (Daily)' },
      description: { type: 'string' },
      source: { type: 'string', enum: ['custom', 'registry'], example: 'custom' },
      readOnly: { type: 'boolean', example: false },
      status: { type: 'string', enum: ['active', 'deprecated'], example: 'active' },
      uiStatus: { type: 'string', enum: ['active', 'deprecated'], example: 'active' },
      fields: {
        type: 'array',
        items: {
          type: 'object',
  ...
  AnalyticsReport: {
    ...
    properties: {
      ...
      ownerId: mongoId,
      orgId: mongoId,
      chartIds: { type: 'array', items: mongoId },
      dashboardId: mongoId,
      datasetId: { type: 'string' },
      outputFormat: { type: 'string', enum: ['csv', 'json'], example: 'csv' },
      status: { type: 'string', enum: ['generated', 'exported', 'archived'], example: 'generated' },
      uiStatus: { type: 'string', enum: ['generated', 'exported', 'archived'], example: 'generated' },
      generatedAt: isoDate,
      exportedAt: isoDate,
      archivedAt: isoDate,
      createdAt: isoDate,
    ...
    properties: {
      ...
      reading: {
        ...
        properties: {
          ...
          isDay: { type: 'boolean' },
          vapourPressureDeficitKPa: { type: 'number' },
          evapotranspirationMm: { type: 'number' },
          et0FaoEvapotranspirationMm: { type: 'number' },
          shortwaveRadiationWm2: { type: 'number' },
          directRadiationWm2: { type: 'number' },
          diffuseRadiationWm2: { type: 'number' },
          directNormalIrradianceWm2: { type: 'number' },
          sunshineDurationSeconds: { type: 'number' },
          soilTemperature0cm: { type: 'number' },
          soilTemperature6cm: { type: 'number' },
          soilMoisture0To1cm: { type: 'number' },
          soilMoisture1To3cm: { type: 'number' },
          soilMoisture3To9cm: { type: 'number' },
        },
      },
